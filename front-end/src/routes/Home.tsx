      ...
      <SidebarContent>
        ...
        <SidebarGroup>
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton asChild>
                <button className="w-full"><User /><span>Profile</span></button>
              </SidebarMenuButton>
            </SidebarMenuItem>
            <SidebarMenuItem>
              <SidebarMenuButton asChild>
                <button className="w-full"><Settings /><span>Settings</span></button>
              </SidebarMenuButton>
            </SidebarMenuItem>
          </SidebarMenu>
        </SidebarGroup>
...
export default function Home() {
  ...
  // Chat state — populated from server when a conversation is loaded
  ...
  // Load a template passed back from the Templates page via router state
  useEffect(() => {
  ...
  // Clear selectedPreset if the stored preset no longer exists (was deleted)
  useEffect(() => {
    if (presets.length > 0 && selectedPreset && !presets.some((p) => p.id === selectedPreset)) {
      setSelectedPreset("");
    }
  ...
  const saveTemplateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (currentTemplateId) {
        return updateTemplate(token!, currentTemplateId, { name, html_content: templateHtml! });
      }
  ...
  const renameMutation = useMutation({
    mutationFn: (name: string) => updateTemplate(token!, currentTemplateId!, { name }),
  ...
  function handleNameKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "Escape") { setTemplateName(nameBeforeEdit.current); setIsEditingName(false); }
  }
  ...
  async function handleSend() {
    ...
    try {
      ...
      // If this was the first message, store the new conversation id and refresh list
          ...
          <div className="flex flex-1 min-h-0">
            ...
            <div className={`flex-col flex-1 md:flex-none w-full md:w-[380px] md:shrink-0 min-h-0 md:border-r border-border ${mobileTab === "chat" ? "flex" : "hidden md:flex"}`}>
              ...
              <div className="shrink-0 p-3 border-t border-border">
                <div className={`rounded-xl border bg-background shadow-sm ${noPreset ? "opacity-60" : ""}`}>
                  <Textarea
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    onKeyDown={handleKeyDown}
                    placeholder={noPreset ? "Select a preset first…" : "Describe the invoice template you want…"}
                    rows={3}
                  ...
                  <div className="flex items-center gap-2 px-3 py-2 border-t border-border">
                      ...
                      <SelectContent>
                        {presets.length === 0 ? (
                          <div className="px-3 py-4 text-xs text-muted-foreground text-center">
                            No presets yet. Create one<br />in the sidebar under Presets.
                          </div>
                        ) : (
                          presets.map((p) => (
                    ...
                    <Button
                      ...
                      disabled={!input.trim() || isLoading || noPreset || loadingConversation}
                    >
                ...
                {noPreset ? (
                  <p className="text-center text-xs text-destructive mt-2">
                    Select a preset to start — or create one under{" "}
                    <button
            ...
            <div className={`flex-col flex-1 min-h-0 min-w-0 ${mobileTab === "preview" ? "flex" : "hidden md:flex"}`}>
              ...
              {templateHtml ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-8">
                  ...
                  <div>
                    <p className="text-sm font-medium">No template yet</p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Describe the invoice you want in the chat and the preview will appear here.
                    </p>
                  </div>
                </div>
              )}
